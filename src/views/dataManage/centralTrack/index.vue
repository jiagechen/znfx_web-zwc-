...
export default {
  ...
  methods: {
    handleDelete(routeID) {
      ...
      request.delete('http://localhost:8080/centraltrack/' + routeID).then(res => {
        if (res.code === '200') {
          this.$message({
            type: 'success',
            message: '删除成功'
    ...
    load() {
      request.get('http://localhost:8080/centraltrack', {
        params: {
      ...
      this.$refs[form].validate((valid) => {
        if (!valid) {
          ...
          if (this.form.routeID) { // id存在，为更新
            request.put('http://localhost:8080/centraltrack', this.form).then(res => {
              console.log(res)
              if (res.code === '200') {
                this.$message({
                  type: 'success',
            ...
            request.post('http://localhost:8080/centraltrack', this.form).then(res => {
              console.log(res)
              if (res.code === '200') {
                this.$message({
                  type: 'success',
